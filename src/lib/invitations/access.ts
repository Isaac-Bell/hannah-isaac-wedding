...
import { RATE_LIMIT_WINDOW_MS } from "@/lib/rate-limit/invitation";
...
export async function authenticateInvitationCode({
  ...
  if (!valid || !invitation) {
    const decision = await store.recordFailedAttemptIfAllowed({
      requesterHash,
      codeFingerprint,
      attemptedAt: now,
    ...
    return {
      ok: false,
      reason: decision.allowed ? "invalid" : "rate-limited",
    };
  }

  await store.recordSuccessfulAttempt({
    requesterHash,
    codeFingerprint,
    attemptedAt: now,
  });
